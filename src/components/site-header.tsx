    ...
    <header className="sticky top-0 z-50 border-b border-[#d6deec] bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex items-center gap-3" onClick={() => setOpen(false)}>
          <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#b10f2e] text-sm font-semibold text-white shadow-lg shadow-[#b10f2e]/20 transition-transform group-hover:-rotate-6">
            VS
          </span>
          <span>
            <span className="block text-sm font-semibold uppercase tracking-[0.24em] text-[#14213d]">Visa Sponsor Jobs</span>
            <span className="block text-xs text-slate-500">Global jobs with external apply links</span>
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-6 md:flex">
          <div className="relative" ref={countriesRef}>
            <button
              type="button"
              className="inline-flex items-center gap-1 text-sm font-medium text-[#334155] transition hover:text-[#b10f2e]"
          ...
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="text-sm font-medium text-[#334155] transition hover:text-[#b10f2e]">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          <Button asChild variant="outline" className="hidden sm:inline-flex border-[#c8d0df] text-[#14213d] hover:border-[#b10f2e] hover:text-[#b10f2e]">
          ...
          <button
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-[#c8d0df] bg-white text-[#14213d] md:hidden"
